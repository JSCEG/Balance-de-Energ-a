document.addEventListener('DOMContentLoaded', () => {
    ...
    const defaultColors = { ...(config.energeticColors || {}) };
    const nodeNames = Object.keys(defaultColors);
    ...
    nodeNames.forEach(name => {
        if (!nodeCategories[name]) {
            nodeCategories[name] = 'otros';
        }
    });
    ...
    const resolvedEnergeticosOverrides = {};
    Object.entries(energeticosPaletteOverrides).forEach(([rawName, color]) => {
        const targetName = defaultNameMap[normalizeName(rawName)];
        if (targetName) {
            resolvedEnergeticosOverrides[targetName] = color;
        }
    });
    ...
    const paletteDefinitions = [
        ...
        },
        {
            ...
            colors: { ...defaultColors, ...resolvedEnergeticosOverrides },
            preview: ['#2B2B2B', '#7B2C2C', '#A94442', '#1E5B4F', '#22C55E', '#FFD54F']
        }
    ];
    ...
    paletteDefinitions.forEach(palette => {
        ...
        if (!palette.preview) {
            ...
            palette.preview = unique.slice(0, 5);
        }
    });

    let currentPaletteId = findMatchingPalette(defaultColors);

    // --- Colores Tab ---
    config.columnas?.forEach(column => {
        column.nodos.forEach(node => {
            if (defaultColors[node.nombre]) {
                const row = document.createElement('tr');

                const colorCell = document.createElement('td');
                colorCell.style.backgroundColor = defaultColors[node.nombre];
                ...
                const pickr = Pickr.create({
                    ...
                    theme: 'classic',
                    default: defaultColors[node.nombre],
                ...
                pickr.on('clear', () => {
                    const fallback = defaultColors[node.nombre];
    ...
    function applyPalette(paletteId) {
        ...
        nodeNames.forEach(name => {
            const color = paletteColors[name] || defaultColors[name] || '#808080';
    ...
    function collectPaletteRows() {
        ...
        config.columnas?.forEach(column => {
            column.nodos.forEach(node => {
                const color = newColors[node.nombre] || defaultColors[node.nombre];
                if (!color) return;
                rows.push({
